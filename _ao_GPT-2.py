...
class GPT2:
    def __init__(self, 数据集地址, bitchSize, GPU=True, senMaxLen=200, mask=True, pretrained_model='gpt2-medium'):
    ...
    def _getAvgWordEmbedding(trainPaper_sentence_L, testPaper_sentence_L, GPU, bitchSize, senMaxLen, mask, pretrained_model):
        ...
        senVectors_L = []
        with torch.no_grad():
            for i in tqdm(range(bsNum), '计算gpt-2平均词向量'):
...
if __name__ == '__main__':
    ...
    GPT2_obj = GPT2(数据集地址=数据集地址, bitchSize=200, GPU=True, senMaxLen=200, mask=True, pretrained_model='gpt2-medium')

    # 检索数据集评估
    # test_train_D = GPT2_obj.计算测试集与训练集余弦相似度()
    # IR评估_obj = IR评估(标签地址=数据集地址)
    # IR评估_obj.评估(预测标签=test_train_D, topN=20, 简化=True, 输出地址='', 输出控制台=True)

    # 分类数据集评估
    test_train_dis_L, trainLabel_L, testLabel_L = GPT2_obj.计算分类数据集相似度矩阵()
