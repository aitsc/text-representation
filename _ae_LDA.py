...
class LDA:
    def __init__(self, 数据集地址, workers, num_topics, iterations):
        self._trainPaper_sentence_L, self._testPaper_sentence_L = self._getPaperInfor(数据集地址)
        self._trainSenVec, self._testSenVec = self._getDocTopicProbability(self._trainPaper_sentence_L, self._testPaper_sentence_L, workers, num_topics, iterations)
    ...
    @staticmethod
    def _getDocTopicProbability(trainPaper_sentence_L, testPaper_sentence_L, workers, num_topics, iterations):
        ...
        if workers <= 1:
            lda = LdaModel(corpus=corpus, id2word=dictionary, num_topics=num_topics, iterations=iterations, minimum_probability=0., minimum_phi_value=0.)
        else:
            lda = LdaMulticore(corpus=corpus, id2word=dictionary, workers=workers, num_topics=num_topics, iterations=iterations, minimum_probability=0., minimum_phi_value=0.)
        trainSenVec = []
        testSenVec = []
        for topic_prob in tqdm(lda.get_document_topics(corpus[:len(trainPaper_sentence_L)]), '获取训练集主题向量'):
            trainSenVec.append([i for _, i in topic_prob])
        for topic_prob in tqdm(lda.get_document_topics(corpus[len(trainPaper_sentence_L):]), '获取测试集主题向量'):
...
if __name__ == '__main__':
    ...
    LDA_obj = LDA(数据集地址=数据集地址, workers=4, num_topics=100, iterations=100)  # 这里的work每个进程都能占满cpu

    # 检索数据集评估
    # test_train_D = LDA_obj.计算测试集与训练集余弦相似度()
    # IR评估_obj = IR评估(标签地址=数据集地址)
    # IR评估_obj.评估(预测标签=test_train_D, topN=20, 简化=True, 输出地址='', 输出控制台=True)

    # 分类数据集评估
    test_train_dis_L, trainLabel_L, testLabel_L = LDA_obj.计算分类数据集相似度矩阵()
