import torch
from pytorch_transformers import OpenAIGPTTokenizer, OpenAIGPTModel
import numpy as np
import math
import time
...
class GPT:
    def __init__(self, 数据集地址, bitchSize, GPU=True, senMaxLen=200, mask=True, pretrained_model='openai-gpt'):
    ...
    def _getAvgWordEmbedding(trainPaper_sentence_L, testPaper_sentence_L, GPU, bitchSize, senMaxLen, mask, pretrained_model):
        tokenizer = OpenAIGPTTokenizer.from_pretrained(pretrained_model)
        model = OpenAIGPTModel.from_pretrained(pretrained_model)
        model.eval()
        if GPU:
            model.to('cuda')
        ...
        senVectors_L = []
        with torch.no_grad():
            for i in tqdm(range(bsNum), '计算gpt平均词向量'):
...
if __name__ == '__main__':
    ...
    GPT_obj = GPT(数据集地址=数据集地址, bitchSize=100, GPU=True, senMaxLen=200, mask=True, pretrained_model='openai-gpt')

    # 检索数据集评估
    # test_train_D = GPT_obj.计算测试集与训练集余弦相似度()
    # IR评估_obj = IR评估(标签地址=数据集地址)
    # IR评估_obj.评估(预测标签=test_train_D, topN=20, 简化=True, 输出地址='', 输出控制台=True)

    # 分类数据集评估
    test_train_dis_L, trainLabel_L, testLabel_L = GPT_obj.计算分类数据集相似度矩阵()
